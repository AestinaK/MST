...
public class NepaliDateConverter :INepaliDateConverter
{
    ...
    {
        var calender = DateConverter.ConvertToNepali(adDate.Year, adDate.Month, adDate.Day);
        return new DateParts(calender.Year, calender.Month, calender.Day);
    }
    ...
    public string GetBsDateFromAdDate(DateTime dateTime, char separator = '-', string format = "yyyy-mm-dd")
    {
        ...
        return GetBsDateFromAdDate(year, month, day);
    }
    ...
    private string GetKey(int yy, int mm, int dd) => $"{yy}- {mm} - {dd}";
}