...
using DateConverter.Converters.Interface;
...
public class DateConverter : IDateConverter
{
    ...
    /// </summary>
    public string WeekNameDay { get; private set; }
    ...
    public static DateConverter ConvertToNepali(int yy, int mm, int dd)
    {
        ...
        if (calender.ValidEnglishDate(yy, mm, dd))
        {
            // english months
            ...
            {
                if (calender.IsLeapYear(def_eyy + i))
                {
                    for (j = 0; j < 12; j++)
                    {
            ...
            converter.WeekNameDay = calender.GetDayOfWeek(day);
            converter.MonthName = calender.GetNepaliMonth(m);
    ...
    public static DateConverter ConvertToEnglish(int yy, int mm, int dd)
    {
        ...
        if (calender.ValidNepaliDate(yy, mm, dd))
        {
            ...
            for (i = 0; i < yy - def_nyy; i++)
            {
                for (j = 1; j <= 12; j++)
                {
                    total_nDays += calender.BSCalendar[k][j];
                }
            ...
            converter.WeekDay = day;
            converter.WeekNameDay = calender.GetDayOfWeek(day);
            converter.MonthName = calender.GetEnglishMonth(m);
